  ...
  return (
    <motion.div
      ...
      className={`rounded-3xl px-2 sm:px-4 py-2 sm:py-4 transition ease-out overflow-hidden ${
        isOpen ? "bg-gray-300/20 dark:bg-gray-400/10" : ""
      }`}
    >
      <motion.dt
        className={`border-b py-2 sm:py-4 text-lg transition ${
          isOpen || isNextOpen
            ? "border-transparent"
            : "border-gray-200 dark:border-gray-700 dark:border-opacity-50 border-opacity-50"
        }`}
      >
        <button
          type="button"
        ...
        >
          <div className="group-focus-visible:outline-blue-200 dark:group-focus-visible:outline-gray-200 flex items-center justify-between rounded-3xl group-focus-visible:outline group-focus-visible:outline-2 group-focus-visible:outline-offset-2">
            <motion.span
              className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl bg-gradient-to-r from-[#0074a1] via-sky-800 to-[#0074a1] bg-clip-text text-transparent dark:text-gray-200 ${TestFont.className}`}
            >
              {service.name}
            </motion.span>
            <span className="ml-2 sm:ml-4 md:ml-8 lg:ml-16 flex h-7 items-center">
              ...
              >
                <rect
                  ...
                  stroke="currentColor"
                  className="dark:stroke-white stroke-sky-800"
                />
                <path
                  ...
                  fill="currentColor"
                  className="dark:fill-white fill-sky-800"
                />
              </svg>
            </span>
          </div>
      ...
      {isOpen && (
        <motion.dd className="pb-6 sm:pb-10 pr-2 sm:pr-6" id={service.id}>
          <p
            className={`bg-gradient-to-r from-gray-800 via-gray-500 to-gray-800 bg-clip-text text-transparent dark:text-gray-300 font-extralight sm:font-normal text-sm sm:text-base md:text-lg ${TestFont.className}`}
          >
            {service.description}
          </p>
        </motion.dd>
...
const Services = () => {
  ...
  return (
    <div className="lg:container w-screen mt-6 lg:mt-20 my-4 sm:my-8 md:my-12 px-2 sm:px-4 md:px-6 lg:px-8">
      <div className="flex flex-col items-center">
        <h4
          className={`text-2xl sm:text-3xl md:text-4xl mt-4 sm:mt-8 md:mt-12 mb-2 sm:mb-10 md:mb-16 text-center bg-gradient-to-r from-[#0074a1] via-sky-800 to-[#0074a1] bg-clip-text text-transparent dark:text-white uppercase ${Engebrechtre.className}`}
        >
          Our Services
        </h4>
      </div>
